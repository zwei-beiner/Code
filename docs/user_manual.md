...
|Wavelengths | {% raw %}{% endraw %}$\char123 \{\lambda_1, \lambda_2,\dots, \lambda_K\}$  | One of <ul><li>`tuple[float, float]`</li><li>`numpy` array</li></ul> | <ul><li>Unit: $\mathrm{meter}$.</li><li>These are the wavelengths in the outer medium. They are related to the wavelength in vacuum by $\lambda_\text{vac}=n_\text{outer}(\lambda)\lambda$.</li><li>If specified as a `tuple`, the input is parsed as the lower and upper limits of an interval, $(\lambda_\text{min}, \lambda_\text{max})$, and the wavelengths $\{\lambda_1, \lambda_2,\dots, \lambda_K\}$ are automatically chosen.</li><li>If specified as a `numpy` array, these are understood to be the wavelengths $\{\lambda_1, \lambda_2,\dots, \lambda_K\}$ themselves.</li></ul>
...
|--|--|--
|1|fixed|$n_1=1.7$
|2|categorical| $n_2\in \{0.3, 2.4\}$
|3|fixed|$n_3=0.9$
|4|fixed |$n_4=0.6$
|5|categorical |$n_5\in\{1.3, 5.6, 3.8\}$
