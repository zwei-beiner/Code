...
from src.reflectivity_pure_python_old import reflectivity
...
class Test_reflectivity(TestCase):
    def test_5_layers(self):
        ...
        def make_merit_function(n: npt.NDArray[np.float_], wavelengths: npt.NDArray[np.float_],
                                target_reflecitivities: npt.NDArray[np.float_]):
        ...
        wavelengths = np.concatenate(
            (np.arange(600, 1000, 20), np.arange(1000, 2000, 40), np.arange(2000, 2300, 50), np.array([2300]))) * 1e-9
        ...
        settings = output_results(loglikelihood, M, prior, 10, True)
    ...
    def test_if_Hobson_Baldwin_antireflection_coating_results_are_reproduced(self):
        ...
        tic = time.process_time()
        settings = output_results(loglikelihood, M, prior, 0, True)
        toc = time.process_time()
        print(f'Elapsed time = {toc - tic} seconds.')
